...
export function ClientCaseDetailPageContent({ initialCase }: { initialCase: Case }) {
  const [caseItem, setCaseItem] = useState<Case | null>(initialCase);
  const [estimate, setEstimate] = useState<Case['_estimate'] | undefined>(initialCase._estimate);
  ...
  useEffect(() => {
    ...
    if (caseItem?._estimate) {
        setEstimate(caseItem._estimate);
  ...
  if (!caseItem) {
    // This case should ideally not be reached if initialCase is always provided
    return notFound();
  }
